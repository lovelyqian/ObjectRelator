    ...
    def preprocess(self, dataset_dict, region_mask_type=None, mask_format='polygon'):
        ...
        if 'vp_image' in dataset_dict:
            if isinstance(dataset_dict["vp_image"], str):
                #通过这种方式读取的图片是一个numpy数组
                # vp_image = utils.read_image(dataset_dict["vp_image"], format='RGB')
                #通过PIL的方式online读取并缩放图片
                img = Image.open(dataset_dict["vp_image"])
                #new_size不能写死，因为有的exo的图片是（960，540）
                image_tmp = utils.read_image(dataset_dict["file_name"], format='RGB')
                h_tmp,w_tmp = image_tmp.shape[:2]
                img_resize = img.resize((w_tmp,h_tmp))
                vp_image = np.array(img_resize)
            ...
            #在进行模型的预设变换前，先将ego放缩到和exo相同的尺寸
            # vp_image = cv2.resize(vp_image, (960,540))
            # print("vp_image_exosize:", vp_image.shape)
            ...
            #在对ego mask进行模型预设处理前，先将其转化为exo的大小
            # for seg in dataset_dict["vp_annotations"]:
            #     # print(seg)
            #     binary_mask = decode(seg["segmentation"])
            #     binary_mask = cv2.resize(binary_mask, (960, 540), interpolation=cv2.INTER_NEAREST)
            ...
            #     }
            #     print("mask_exoshape:", segmentation["size"])

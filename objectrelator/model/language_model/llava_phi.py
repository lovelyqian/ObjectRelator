...
from ..mask_decoder.Mask2Former_Simplify.modeling.transformer_decoder.ObjectRelator_decoder import MultiScaleMaskedTransformerDecoder
...
def fuse_multicondition(fuse_method, weight, SEG_embedding, region_embedding_list):
        ...
        for seg_embed, region_embedding in zip(SEG_embedding, region_embedding_list):
            if fuse_method == "add":
                if weight is not None:  
                    fused_region = weight * seg_embed + (1 - weight) * region_embedding
                else:
            ...
            elif fuse_method == "cross_attention_withoutpara":
                attention_scores = F.softmax(seg_embed @ region_embedding.transpose(-2, -1), dim=-1)  
                fused_region = (attention_scores.transpose(-2, -1) * region_embedding)  
            
            new_region_embedding_list.append(fused_region)
       
        return new_region_embedding_list

def deep_copy_input(input_ids, attention_mask, past_key_values, labels, images, vp_images, 
                  class_name_embedding_indices, class_name_ids, cls_indices, instances, 
                  token_refer_id, refer_embedding_indices,output_attentions):
    
    def copy_if_tensor(value):
        if value is None or isinstance(value, bool):
            return value  # Directly return None or bool without modification
        elif isinstance(value, torch.Tensor):
            # Copy the tensor, and set requires_grad only if the original tensor requires it and is floating-point
            new_value = value.detach().clone()
            if value.requires_grad:
                new_value.requires_grad_(True)
            return new_value
        elif isinstance(value, list):
            # Deep copy each tensor in the list, if applicable
            return [copy_if_tensor(v) for v in value]
        else:
            return value  # If it's not tensor or list, return it directly
        
    # Apply the helper function to each input and create the corresponding _exo version
    input_ids_exo = copy_if_tensor(input_ids)
    attention_mask_exo = copy_if_tensor(attention_mask)
    past_key_values_exo = copy_if_tensor(past_key_values)
    labels_exo = copy_if_tensor(labels)
    images_exo = copy_if_tensor(images)
    vp_images_exo = copy_if_tensor(vp_images)
    class_name_embedding_indices_exo = copy_if_tensor(class_name_embedding_indices)
    class_name_ids_exo = copy_if_tensor(class_name_ids)
    cls_indices_exo = copy_if_tensor(cls_indices)
    instances_exo = copy_if_tensor(instances)
    token_refer_id_exo = copy_if_tensor(token_refer_id)
    refer_embedding_indices_exo = copy_if_tensor(refer_embedding_indices)
    output_attentions_exo = copy_if_tensor(output_attentions)

    # Return all the copied values as a dictionary or as individual values
    return input_ids_exo, attention_mask_exo, past_key_values_exo, labels_exo, images_exo, vp_images_exo, class_name_embedding_indices_exo, class_name_ids_exo, cls_indices_exo, instances_exo, token_refer_id_exo, refer_embedding_indices_exo, output_attentions_exo


def XObjAlign(embedding_list1, embedding_list2, sim_type="cos", L2_norm=False):
...
class ObjectRelator(PhiForCausalLM, LlavaMetaForCausalLM):
    print('hi, here, class ObjectRelator')
    config_class = LlavaConfig

    def __init__(self, config, mask_decoder_cfg=None, add_cross_attn=True, cross_attn_index=None):
        super(ObjectRelator, self).__init__(config)
    ...
    def prepare_inputs_labels_for_multimodal_EXO(
            self, input_ids, attention_mask, past_key_values, labels, images, vp_images=None,
            class_name_embedding_indices=None,
            class_name_ids=None, cls_indices=None, instances=None, token_refer_id=None, refer_embedding_indices=None
    ):
        ...
        if (input_ids == REGION_TOKEN_INDEX).sum() != 0 and instances is not None:
            ...
            region_masks_list = [instance.gt_masks.detach().clone() for instance in instances]
            #image_features_2 = image_features.detach().clone().requires_grad_(True)
    ...
    def forward(
        ...
        if (input_ids == SEG_TOKEN_INDEX).sum() != 0:
            ...
            input_ids_exo, attention_mask_exo, past_key_values_exo, labels_exo, images_exo, vp_images_exo, class_name_embedding_indices_exo, class_name_ids_exo, cls_indices_exo, instances_exo, token_refer_id_exo, refer_embedding_indices_exo, output_attentions_exo = deep_copy_input(
                input_ids, attention_mask, past_key_values, labels, images, vp_images, class_name_embedding_indices,class_name_ids, cls_indices, instances, token_refer_id, refer_embedding_indices, output_attentions
                )
        ...
        loss_region_emb_SSL = XObjAlign(region_embedding_list, region_embedding_list_exo, sim_type="ecu") #ours
        ...
        if seg_query_mask is not None:
            ...
            if seg_info is not None:
                ...
                # adjust the SSL loss weights
                k_SSL = 1
                mask_loss = loss_mask + loss_dice + loss_SEG_class + loss_class_name_class + loss_region_class  + k_SSL*loss_region_emb_SSL

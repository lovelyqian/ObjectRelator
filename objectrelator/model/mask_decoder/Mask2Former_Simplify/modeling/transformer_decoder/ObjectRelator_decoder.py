    ...
    def fuse_multicondition(self, SEG_embedding, region_embedding_list):
        ...
        for seg_embed, region_embedding in zip(SEG_embedding, region_embedding_list):
            if self.fuse_method == "add":
                ...
                attention_scores = F.softmax(region_embedding @ seg_embed.transpose(-2, -1), dim=0)  
                fused_seg = (attention_scores * seg_embed).sum(dim=0, keepdim=True)  
...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    def __init__(
        ...
        #CA method,  with learnable residual connection, num_heads =1 
        if data_args.condition == 'multi-condition':
            self.fuse_model = MCFuse(dim=hidden_dim, fuse_method="CA", learnable_weight=True, num_heads=1) 

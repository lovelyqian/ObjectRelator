...
if __name__ == '__main__':
    ...
    # 实验需改动
    save_path = os.path.join(root_path, 'ExoQuery_piano.json')
    ...
    for val_name in tqdm(val_set):
        ...
        # 取exo视角下出现时间最长的物体对应的所有注释帧，作为基准帧
        obj_ref = objs_both_have[0]
        for obj in objs_both_have:
            if len(list(annotations["masks"][obj_ref][exo].keys())) < len(list(annotations["masks"][obj][exo].keys())):
                obj_ref = obj
        exo_anno_frames = natsorted(list(annotations["masks"][obj_ref][exo].keys()))
        # TODO给frames排个序
        frames = natsorted(np.intersect1d(exo_anno_frames, ego_frames))
        ...
        for idx in frames:
            ...
            filename = f"{idx}.jpg"
            sample_img_path = os.path.join(vid_ego_path, filename)
            sample_img_relpath = os.path.relpath(sample_img_path, root_path)

            first_frame_img_path = os.path.join(vid_exo_path, filename)
            ...
            for obj in objs_both_have:
                if idx in annotations["masks"][obj][exo].keys():
                    ...
                    area_new = mask_exo.sum().astype(float)
                    if area_new != 0:
            ...
            height, width = annotations["masks"][obj_list_ego[0]][ego][idx]["size"]
            ...
            for obj in obj_list_ego:
                assert obj in obj_list_exo_new, 'Found new target not in the first frame'
            ...
            sample_unique_instances = [float(coco_id_to_cont_id[obj]) for obj in obj_list_ego_new]
